<?php
	...
	class loginControlador extends mainModel{
		...
		public function iniciar_sesion_user_controlador(){
			...
			/*-- Verificando datos de la cuenta - Verifying account details --*/
			$datos_cuenta=mainModel::datos_tabla("Normal","cliente WHERE cliente_email='$usuario' AND cliente_clave='$clave' AND cliente_cuenta_estado='Activa'","*",0);
			...
			if($datos_cuenta->rowCount()==1){
				...
				$_SESSION['estado_sto']=$row['cliente_cuenta_estado'];
		...
		public function cerrar_sesion_user_controlador(){
			
			

			if(estado_sto==$_SESSION['Activa']){
				unset($_SESSION['id_sto']);
				unset($_SESSION['nombre_sto']);
				unset($_SESSION['apellido_sto']);
				unset($_SESSION['genero_sto']);
				unset($_SESSION['email_sto']);
				unset($_SESSION['estado_sto']);
